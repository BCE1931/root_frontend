...
export default function App() {
  ...
  }, [isLoading, treeData.length, updateScrollState, scheduleScrollStateUpdate]);
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      setTreeData(data);
      toast.info("Tree reloaded from database!");
    } catch {
      toast.error("Failed to reload from backend");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const addRecursively = (nodes, parentId, newNode) =>
    nodes.map((n) => {
      ...
      if (n.children.length > 0)
        return { ...n, children: addRecursively(n.children, parentId, newNode) };
      return n;
    });
  ...
  const handleEditNode = async (nodeId, newText) => {
    ...
    if (!node) return;
    try {
      await api.updateNode(nodeId, { text: newText, description: node.description });
      const editRec = (nodes) =>
        nodes.map((n) => {
          if (n.id === nodeId) return { ...n, text: newText };
          if (n.children.length > 0) return { ...n, children: editRec(n.children) };
          return n;
        });
  ...
  const handleUpdateDescription = async (nodeId, newDescription) => {
    ...
    if (!node) return;
    try {
      await api.updateNode(nodeId, { text: node.text, description: newDescription });
              ...
              <div className="app-container">
                {isLoading ? (
                  ...
                  <div className="tree-viewport">
                    ...
                    <div ref={treeWrapperRef} className={`tree-wrapper ${layout}`}>
                      {treeData.map((node) => (
                        <TreeNode
                          key={node.id}
                          node={node}
